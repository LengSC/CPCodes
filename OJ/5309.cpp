...
namespace SLV {
    ...
    constexpr ll N = 1e5 + 5, M = 1e7, OFS = 1e15, RANGE = 2e15;

    ll n, maxn, minn, a[N], ans;
    ...
    int main() {
        ...
        per(i, n, 1) {
            SGT::insert(SGT::root, 1, RANGE, a[i] + OFS);
            ans += SGT::qry(SGT::root, 1, RANGE, 1, a[i - 1] + OFS + maxn) - SGT::qry(SGT::root, 1, RANGE, 1, a[i - 1] + OFS + minn - 1);
        }
        printf("%lld\n", ans);
        return 0;
    }
